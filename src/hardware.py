...
class HardwareController:
    ...
    def run_pir_servo_sequence(self):
        ...
        try:
            logger.info("[PIR 센서 서보 제어] 서보모터 회전: 원래 위치 -> 35도 (부드럽게 이동)")
            if self.has_hw:
                self.led_action.on()
                self._set_servo_smooth(0.060, duration=0.5)
            else:
                logger.info("[가상 서보 모터] 위치: 35도 이동")
    ...
    def run_ventilation(self):
        ...
        try:
            ...
            time.sleep(0.5)
            # 35도까지 부드럽게 폐쇄
            self._set_servo_smooth(0.060, duration=0.8)
            time.sleep(0.5)
